...
export function MultiDeviceMabarModal({ onStartBattle, onClose }: MultiDeviceMabarModalProps) {
  const [roomCodeInput, setRoomCodeInput] = useState('4C');
  ...
  useEffect(() => {
    ...
    const targetCode = (codeParam ? codeParam.toUpperCase() : '4C').trim();
  ...
  const handleCreateOrJoinRoom = async (overrideCode?: string) => {
    soundManager.playButton();
    const cleanCode = (overrideCode || roomCodeInput || '4C').toUpperCase().trim();
  ...
  const handleConfirmJoinGroup = async () => {
    ...
    const cleanCode = (roomCodeInput || '4C').toUpperCase().trim();
    ...
    await multiplayerClient.createOrJoinRoom(cleanCode);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-slate-950/85 backdrop-blur-md animate-fadeIn">
      <div className="w-full max-w-4xl max-h-[92vh] bg-slate-900 border border-indigo-500/40 rounded-3xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
          ...
          <div className="bg-slate-850 border border-slate-800 rounded-2xl p-4 flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full md:w-auto">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {/* Preset quick room buttons */}
                <div className="flex items-center gap-1 bg-slate-900 border border-slate-800 p-1 rounded-xl">
                  {['4C', '4A', '4B', '4D'].map((presetCode) => (
                    <button
                      key={presetCode}
                      onClick={() => handleCreateOrJoinRoom(presetCode)}
                      className={`px-2.5 py-1 rounded-lg text-xs font-bold font-mono transition ${
                        roomCodeInput === presetCode
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') handleCreateOrJoinRoom();
                    }}
                    placeholder="4C"
                    className="px-3 py-1.5 bg-slate-900 border border-indigo-500/40 rounded-xl font-mono font-black text-sm text-indigo-300 w-24 uppercase text-center focus:outline-hidden focus:ring-2 focus:ring-indigo-400"
                  />
                  <button
            ...
            <div className="flex items-center gap-2 w-full md:w-auto justify-end">
              ...
              <div className="px-3 py-1 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-xs font-bold flex items-center gap-1.5">
                ...
                <span>{groupsList.length} Tim di Ruang {roomCodeInput}</span>
              </div>
            </div>
          </div>
          ...
          {isJoined && (
            <div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-950/80 via-slate-900 to-slate-900 border border-emerald-500/50 shadow-xl flex flex-col sm:flex-row items-center justify-between gap-3 animate-fadeIn">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="px-2 py-0.5 rounded-md bg-amber-400/20 text-amber-300 font-mono text-[11px] font-black border border-amber-400/30">
                      Ruang {roomCodeInput}
                    </span>
                  </div>
                  <p className="text-xs text-white font-medium mt-0.5">
                    Device ini aktif sebagai <strong>{PRESET_GROUPS.find((g) => g.id === selectedGroupId)?.name}</strong>. Kelompokmu sudah tampil di Dasbor Guru!
                  </p>
                </div>
              </div>
              <button
                onClick={handleHostStart}
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-indigo-600 hover:from-emerald-400 hover:to-indigo-500 text-white font-black text-xs shadow-lg flex items-center gap-2 transition active:scale-95 shrink-0"
              >
